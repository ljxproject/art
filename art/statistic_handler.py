...
def ArtEditGetHandler(request):
	...
	if id != None:
		logger.warning("ArtEditGetHandler,id:" + str(id))
		id = int(id)
		...
		logger.warning("ArtEditGetHandler::id is None")
		context = {
			'tags': tags,
			'id': id,
		}
...
def ArtEditPostHandler(request):
	logger.warning('#######ArtEditPostHandler::POST')
	import json
	...
	if res["ok"] == 1:
		if int(id) == 0:
			logger.warning("res ok with id == 0, new art.")
			...
			# self.db.commit()
			# self.db.close()
			logger.warning("res ok with the exist art.")
